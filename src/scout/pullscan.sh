...
OUTPUT_FILE="/workspaces/images/src/scout/pull-scan-results.json"
OUTPUT_SUMMARY="/workspaces/images/src/scout/pull-scan-summary.txt"
...
IMAGES=(
   ...
   # "miniconda:3|linux/amd64,linux/arm64"
    "base:alpine3.23,alpine3.22,alpine3.21,alpine3.20,alpine|linux/amd64,linux/arm64"
...
for image_spec in "${IMAGES[@]}"; do
    ...
    while IFS= read -r variant; do
        ...
        while IFS= read -r arch; do
            ...
            echo "Scanning..."
            docker scout cves "$IMAGE_TAG" --platform "$arch" --only-severity critical,high > "$TEMP_SCAN_FILE" 2>&1 || true
